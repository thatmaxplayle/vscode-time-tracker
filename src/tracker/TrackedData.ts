...
export class TrackedData {
  ...
  private updateTotalTime() {
    this._totalTime = this._sessions
      .map((s) => s.duration)
      .reduce((acc, d) => (acc += d));
  }
    ...
    if (fs.existsSync(this._storageFile)) {
      try {
        ...
        vscode.window.showErrorMessage(
          `Unable to read stored time tracking data due to: ${error.message}`,
        );
      }
    } else {
      this._sessions = [];
    ...
    try {
      fs.writeFileSync(
        ...
        JSON.stringify(
          ...
          null,
          vscode.workspace.getConfiguration("timetracker").dotTimeTrackerIndent,
        ),
        {
          encoding: "utf-8",
        },
      ...
      vscode.window.showErrorMessage(
        `Unable to store time tracking data due to: ${error.message}`,
      );
    }
  }
}
